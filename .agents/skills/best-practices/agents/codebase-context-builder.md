...
**Pattern Recognition:**
- If adding a feature â†’ Find existing features with similar structure
- If fixing a bug â†’ Find how similar bugs were fixed
- If refactoring â†’ Find well-structured code to emulate
- If testing â†’ Find existing test patterns
...
1. **Check CLAUDE.md** first â€” project-specific instructions
2. **Check README.md** â€” architecture overview
3. **Check package.json/Gemfile** â€” dependencies and scripts
4. **Glob for relevant directories** â€” find where domain code lives

### Phase 3: Deep Exploration
1. **Search by domain keywords** â€” find all related code
2. **Find test files** â€” understand testing patterns
3. **Find similar implementations** â€” code to reference
4. **Check imports/dependencies** â€” understand the tech stack
...
1. **Prioritize findings** â€” most relevant first
2. **Create specific references** â€” exact file paths, line numbers
3. **Formulate suggestions** â€” what to add to the prompt
4. **Note constraints** â€” what the prompt should include
...
- `src/path/to/main.ts` â€” [what it does, why relevant]
- `src/path/to/related.ts:42` â€” [specific function/class]

**Test files:**
- `tests/path/to/main.test.ts` â€” [existing test coverage]
- `tests/helpers/testUtils.ts` â€” [test utilities to use]

**Config files:**
- `src/config/relevant.ts` â€” [relevant configuration]
...
â””â”€â”€ helpers/        # [available test utilities]
```
