...
|------|---------------|--------------|----------------------|
| Bug Fix | Debug examples, error handling | Symptom â†’ location â†’ test â†’ fix | "fix the bug" (no symptom) |
| Feature | Feature impl examples | Pattern reference â†’ scope â†’ tests | "add X" (no constraints) |
| Refactor | Restructuring examples | Preserve behavior â†’ incremental â†’ verify | "make better" (no criteria) |
| Testing | Test writing examples | Edge cases â†’ patterns â†’ coverage | "add tests" (no cases) |
| Performance | Optimization examples | Profile â†’ identify â†’ fix â†’ measure | "make faster" (no target) |
...
| **4. Structure in Phases** | Complex task | Explore â†’ Plan â†’ Implement â†’ Verify |
