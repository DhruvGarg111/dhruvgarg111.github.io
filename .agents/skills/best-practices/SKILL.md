...
# Best Practices â€” Prompt Transformer
...
â†’ Ask using AskUserQuestion:
  - **Question:** "How should I improve this prompt?"
  - **Header:** "Mode"
  - **Options:**
    1. **Transform directly** â€” "I'll apply best practices and output an improved version"
    2. **Build context first** â€” "I'll gather codebase context and intent analysis first"

**User asks to learn/understand:**
â†’ Show the 5 Transformation Principles section

**User asks for examples:**
â†’ Link to references/before-after-examples.md

**User asks to evaluate a prompt:**
â†’ Use the Success Criteria eval rubric at the end of this document
...
1. **Synthesize findings** â€” Combine intent + best practices + codebase context
2. **Apply matching patterns** â€” Use examples from best-practices-referencer as templates
3. **Ground in codebase** â€” Add specific file paths from codebase-context-builder
4. **Transform the prompt** â€” Apply the 5 principles with all gathered context
5. **Output** â€” Show improved prompt with before/after comparison

### Agent Definitions

The agents are defined in `agents/`:
- `agents/task-intent-analyzer.md` â€” Analyzes intent, gaps, and edge cases
- `agents/best-practices-referencer.md` â€” Finds relevant examples and patterns from references/
- `agents/codebase-context-builder.md` â€” Explores codebase for files and conventions
...
1. **Identify what's missing** â€” Check against the 5 principles below
2. **Add missing elements** â€” Verification, context, constraints, phases, rich content
3. **Output the improved prompt** â€” In a code block, ready to copy-paste
4. **Show what changed** â€” Brief comparison of before/after
...
AFTER:  "write a validateEmail function. test cases: user@example.com â†’ true,
         invalid â†’ false, user@.com â†’ false. run the tests after implementing"
```
...
- [ ] **Verification** â€” How to know it worked (tests, screenshot, output)
- [ ] **Location** â€” Specific files, functions, or areas
- [ ] **Constraints** â€” What NOT to do
- [ ] **Single task** â€” Not compound (split if needed)
- [ ] **Phases** â€” If complex, structured as explore â†’ plan â†’ implement
- [ ] **Root cause** â€” For bugs: "address root cause, don't suppress"
- [ ] **CLAUDE.md** â€” Respect project conventions if they exist
...
Don't interrogate â€” one question is enough. Transform with what you learn.
...
## Success Criteria â€” Prompt Quality Eval

A well-transformed prompt passes these checks:

### Principle 1: Verification âœ…
| Check | Pass | Fail |
|-------|------|------|
...
### Principle 4: Structure âœ…
| Check | Pass | Fail |
|-------|------|------|
| Single task | One clear objective | Multiple goals |
| Phased (if complex) | "explore â†’ plan â†’ implement" | Jump straight to code |
| Appropriate depth | Matches task complexity | Over/under-specified |

### Principle 5: Rich Content âœ…
| Check | Pass | Fail |
|-------|------|------|
...
|-------|---------|-------------------|
| â­â­â­â­â­ | Excellent | All 5 |
| â­â­â­â­ | Good | 4 of 5 |
| â­â­â­ | Acceptable | 3 of 5 |
| â­â­ | Needs work | 2 of 5 |
| â­ | Poor | 1 or 0 |

**Target:** Every transformed prompt should score â­â­â­â­ or â­â­â­â­â­
...
- [Best Practices for Claude Code](https://code.claude.com/docs/en/best-practices) â€” Official documentation
- [Claude Code Skills](https://code.claude.com/docs/en/skills) â€” Skill authoring guide
- [Anthropic Prompt Engineering](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering) â€” General prompting patterns
- [Dicklesworthstone meta_skill](https://github.com/Dicklesworthstone/meta_skill) â€” "THE EXACT PROMPT" pattern
